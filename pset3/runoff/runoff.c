...
bool print_winner(void)
{
    float tippingPoint = (float) voter_count / 2;
    // Go through candidates votes.
    for (int i = 0; i < candidate_count; i++) {
        if (candidates[i].votes > tippingPoint) {
            printf("%s\n", candidates[i].name);
            return true;
        }
    }
